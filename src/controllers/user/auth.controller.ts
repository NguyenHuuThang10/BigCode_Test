...
import { LoginRequest, RegisterRequest, AuthTokenResponse, MeResponse } from '../../model/dto/auth.dto';
...
export class UserAuthController extends Controller {
  ...
  public async me(@Request() req: any): Promise<ApiResponse> {
    const user = req.user as { id: string; email: string; name: string; roles: string[] };
    const me: MeResponse = { id: user.id, email: user.email, name: user.name, roles: user.roles as any };
    return Success(me);
  }
}
